...
/**
 * Glatko Sağlık — H5a phone + OTP helpers (server-only).
 *
 * One place owns: E.164 normalization (Montenegro default), the salted phone
 * hash used as the patients/otp lookup key, and 6-digit OTP code generation +
 * peppered hashing. The OTP plain code is NEVER persisted — only otpCodeHash()
 * goes to the DB (health.otp_codes.code_hash). The pepper is a server secret
 * (not in the DB), so a DB leak alone cannot brute-force codes/phones.
 */

const PEPPER =
  process.env.HEALTH_OTP_PEPPER || process.env.RATE_LIMIT_SALT || "glatko-health-dev-pepper";
...
/** Stable, peppered SHA-256 of the E.164 phone — the otp/patient lookup key. */
