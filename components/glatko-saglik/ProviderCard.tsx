 ...
 * the whole card links to the provider profile, no client JS. Localized labels
 * are passed in (the parent page owns the translator), keeping this a plain
 * presentational component. Accent (sky / brandHealth) is wayfinding only — the
 * "Verified" badge + the available-slot placeholder; name/links stay neutral.
 *
 * The available-slot row is laid out NOW (a labelled placeholder) so H4 can drop
 * real clickable time chips into the exact same slot without a redesign.
 */
export function ProviderCard({
  ...
  labels: { verified: string; availabilitySoon: string };
}) {
  return (
    <Link
    ...
    >
      ...
      {/* Available-slot row — H4 fills this with clickable time chips; the
          layout is reserved now so no redesign is needed then. */}
